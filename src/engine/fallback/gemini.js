...
async function gemininFallback(errorText, language) {
  const sanitized = sanitizeError(errorText);
  const prompt = buildPrompt(sanitized, language);
    ...
    try {
      ...
      if (isLastAttempt) {
        return {
          ...
          where: 'See error above',
          why: 'Pattern engine had no match and Gemini API call failed',
          fix: 'Copy the error above and search on Stack Overflow or paste into ChatGPT',
          type: 'AnalysisFailed',
...
module.exports = { gemininFallback };